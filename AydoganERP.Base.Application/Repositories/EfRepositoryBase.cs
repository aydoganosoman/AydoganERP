using AydoganERP.Base.Application.Dynamic;
...
public class EfRepositoryBase<TEntity, TDestiny, TEntityId, TContext> : IAsyncRepository<TEntity, TDestiny, TEntityId>, IQuery<TEntity>, IRepository<TEntity, TDestiny, TEntityId> where TEntity : Entity<TEntityId> where TContext : DbContext
{
    ...
    protected virtual void EditRelationEntityPropertiesToCascadeSoftDelete(AuditableEntity entity)
    {
        entity.LastModified = DateTime.UtcNow;
    }

    protected virtual bool IsSoftDeleted(AuditableEntity entity)
    {
        return entity.LastModified.HasValue;
    }

    private async Task setEntityAsSoftDeleted(AuditableEntity entity,
        bool isAsync = true,
        CancellationToken cancellationToken = default(CancellationToken),
        bool isRoot = true)
    {
        ...
        foreach (INavigation item in list)
        {
            ...
            if (item.IsCollection)
            {
                ...
                {
                    await setEntityAsSoftDeleted((AuditableEntity)item2, isAsync, cancellationToken, isRoot: false);
                }
            ...
            await setEntityAsSoftDeleted((AuditableEntity)obj, isAsync, cancellationToken, isRoot: false);
        }
