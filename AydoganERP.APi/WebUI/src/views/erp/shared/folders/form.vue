...
import { FolderDocumentTypeOptionList } from "@/models/const";
...
function initDocumentTypes() {
  ...
  FolderDocumentTypeOptionList.forEach(opt => {
...
function updateField<K extends keyof FolderFormData>(
  key: K,
...
function updateDocumentTypes() {
  const newValue = selectedDocumentTypes.value.reduce(
    (acc, val) => acc | val,
    0
  );
  ...
  >
    ...
    <el-form-item label="Belge Tipleri">
      <el-checkbox-group
        ...
        @change="updateDocumentTypes"
      >
        <el-checkbox
          v-for="opt in FolderDocumentTypeOptionList"
          :key="opt.value"
          :value="opt.value"
        >
          {{ opt.label }}
